...
-- Keep this in sync with migrations/0001..0124 when table shapes change.
...
-- Mailing lists, sending profiles, and templates (Spec 013, migration 0124).
-- RLS, immutable-root and merge-fence triggers, grants, and the permission catalog live in
-- the migration; this structural mirror exists for sqlc only.
...
CREATE TABLE list_subscriber (
    ...
    FOREIGN KEY (list_id, tenant_root_id) REFERENCES mailing_list (id, tenant_root_id) ON DELETE CASCADE,
    FOREIGN KEY (contact_id, tenant_root_id) REFERENCES contact (id, tenant_root_id),
    CHECK (consent_source = 'public_form' OR consent_attested_by IS NOT NULL),
    CHECK (
...
CREATE TABLE subscriber_tag (
    ...
    FOREIGN KEY (business_id, tenant_root_id) REFERENCES business (id, tenant_root_id),
    FOREIGN KEY (list_id, tenant_root_id) REFERENCES mailing_list (id, tenant_root_id) ON DELETE CASCADE,
    FOREIGN KEY (subscriber_id, tenant_root_id) REFERENCES list_subscriber (id, tenant_root_id) ON DELETE CASCADE
);
