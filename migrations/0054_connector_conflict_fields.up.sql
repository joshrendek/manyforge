-- 0054: generalize connector conflict detection (Spec 004, manyforge-a7j.9). 0046 audited
-- 'connector.conflict.resolved' / external_wins only when the STATUS scalar both-changed.
-- This re-defines sync_inbound_external_issue to also detect priority and subject divergence
-- and to emit a single audit row whose old/new payloads carry every field that conflicted.
-- External-wins behavior (the upsert) is unchanged; only the conflict audit is widened.
