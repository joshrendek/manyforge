-- Revert T049: restore the pre-0022 ingest_inbound_message body — the EXACT 0017
-- (18-arg, p_ticket_id) definition WITHOUT the three reopen-audit additions (no
-- v_old_status declaration, no SELECT ... FOR UPDATE capture, no status_changed
-- audit INSERT). The signature is unchanged, so CREATE OR REPLACE is sufficient
-- and preserves grants; the REVOKE/GRANT below mirror the 0017 pattern.
