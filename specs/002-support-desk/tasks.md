...
- [X] T064 [P] [US5] Integration test `internal/ticketing/audit_integration_test.go`: every support mutation (ingestion, reply, note, status/priority/tag/assignee change, address/domain config, redact) writes an in-tx `audit_entry` with actor/source + before/after (SC-005). 9 principal-bearing mutations in `internal/ticketing/audit_integration_test.go::TestSupportAuditMatrix`; 3 principal-less ingest audits in `internal/inbox/audit_integration_test.go::TestSupportIngestAuditMatrix` (actor NULL + `inputs->>'source'`). `redact`/`ticket.redacted` is folded in by T066.
...
- [X] T066 [US5] `tickets.delete` soft-delete/redact: redact-in-place (`ticket.redacted_at`, blank PII-bearing columns, schedule attachment-blob deletion via outbox), excluded from lists/gets (`WHERE redacted_at IS NULL` → 404 to non-privileged); audited in-tx (research R7, data-model decision). Built in `internal/ticketing/redact.go` (`Service.RedactTicket`) under `WithPrincipal` (a principal is present → no DEFINER needed; the ticket/message/attachment RLS policies authorize the holder's writes — simpler than 001's principal-less erasure proc). Soft UPDATE only (never hard DELETE). Audit `ticket.redacted` carries SCOPE METADATA ONLY (`{message_count, attachment_count}` / `{redacted_at}`) — zero PII retained. Attachment blobs purged out-of-band via `attachment.purge` outbox → `AttachmentPurgeSubscriber` (idempotent `blob.Delete`). HTTP: `DELETE /businesses/{id}/tickets/{tid}` gated `tickets.delete` (204; no-oracle 404). Tests: `redact_integration_test.go`, `attachment_purge_subscriber_test.go`, `redact_pin_test.go`; folded into the T064 audit matrix + T067 no-oracle parity; contract pin `TestRedactTicketEndpointContract`.
- [X] T067 [US5] Verify every list/get query in `inbox`/`ticketing` carries both the app `business_id`/`tenant_root_id` predicate and relies on RLS; confirm no 403-vs-404 distinction anywhere (no-oracle audit of handlers). Behavioral: `internal/ticketing/oracle_integration_test.go::TestNoOracleHTTP404Parity` (unknown/cross-tenant/forbidden GET → byte-identical 404 through the real router). Pins: `internal/security_regression/oracle_collapse_pin_test.go` (no `StatusForbidden` in any ticketing/inbox handler; renderer collapses `ErrForbidden`→404). Predicate coverage already pinned in `support_isolation_pin_test.go` (T031/T062).
...
- [X] T068 [P] Contract suite (`make contract-test`): assert the shared-layer interfaces (`InboundSource`, `Blob`, `Notifier`, event-bus) and the ~15 new endpoints against `contracts/openapi.yaml`; extend the OpenAPI-drift gate in `cmd/drift_test.go`
