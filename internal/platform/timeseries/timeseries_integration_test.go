...
func TestRollup_FailureDoesNotSkipIndependentLaterRollups(t *testing.T) {
	...
	if err := tdb.Super.QueryRow(ctx,
		...
		[]string{"analytics_pageviews", "analytics_dimensions", "analytics_properties", "analytics_site_health"}).Scan(&completed); err != nil {
		t.Fatalf("read later watermarks: %v", err)
	}
	if completed != 4 {
		t.Fatalf("only %d later rollups completed, want 4", completed)
	}
	...
	for _, state := range []string{"analytics_pageviews", "analytics_dimensions", "analytics_properties", "analytics_site_health"} {
