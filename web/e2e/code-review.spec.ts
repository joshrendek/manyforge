...
// NOTE ON REORDER TESTING.
//
// These specs used to drive a physical CDK drag with page.mouse. That test was chased through four
// real causes (a nudge below CDK's 5px dragStartThreshold, page.mouse not auto-scrolling, layout
// shifting on pickup, dropping outside the cdkDropList) and still would not hold: it passed 20/20
// on macOS and failed in headless CI, because the sort decision turns on which side of a row's
// midpoint the pointer lands and that moves with font metrics.
//
// What the test is FOR is that reordering works and persists. That outcome is reachable
// deterministically through the ↑/↓ buttons, which is also the accessible path — drag is not
// keyboard-operable, so the buttons are what keyboard and screen-reader users actually use, and
// they had no coverage at all while the flaky drag test absorbed the attention.
//
// So: the outcome is asserted through the buttons, and the drag WIRING (drop list, handles, their
// aria-labels) is asserted structurally. What is given up is end-to-end proof of CDK's own drag
// behaviour — which is Angular's code, not ours.
...
test('review setup: reorder the reviewbot fallback chain, promoting a fallback to primary', async ({
  ...
  // ...and the first entry can no longer be promoted.
  await expect(page.getByTestId('chain-up-0')).toBeDisabled();
});

test('review setup: promote a fallback to primary in the provider priority list', async ({
  page,
}) => {
  await page.route('**/api/**', (r) => r.fulfill({ json: { items: [], next_cursor: null } }));
  await auth(page);
