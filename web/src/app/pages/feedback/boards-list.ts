...
@Component({
  ...
  template: `
    <div class="mf-card" data-testid="feedback-boards-page">
      <mf-page-header
        [eyebrow]="businessName()"        title="Feedback boards"
...
export class FeedbackBoardsListComponent implements OnInit {
  ...
  reload(): void {
    ...
    this.feedback.listBoards(biz).pipe(takeUntil(this.businessChanged), takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (r) => {
  ...
  create(): void {
    ...
    this.feedback.createBoard(this.businessId(), { name, is_public: this.newPublic }).pipe(takeUntil(this.businessChanged), takeUntilDestroyed(this.destroyRef)).subscribe({
      next: () => {
        this.newName = '';
        this.newPublic = false;
        this.creating.set(false);
