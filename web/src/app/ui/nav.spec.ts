...
describe('NAV_ITEMS', () => {
  it('includes dashboard, support, approvals, connectors, ai credentials, agents, code review, contacts, companies, mcp and accounting with testids', () => {
    const routes = NAV_ITEMS.map((n) => n.route);
    expect(routes).toEqual(['/dashboard', '/support', '/approvals', '/credentials/connector', '/credentials/ai', '/agents', '/code-review', '/crm/contacts', '/crm/companies', '/mcp', '/accounting']);
